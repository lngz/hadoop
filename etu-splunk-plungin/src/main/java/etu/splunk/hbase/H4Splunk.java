...
public class H4Splunk {

	static Configuration conf = HBaseConfiguration.create();
	private static final Logger logger = Logger.getLogger(H4Splunk.class);
			...
			for (Result rs : rsList) {
				...
				for (KeyValue kv : rs.raw()) {
					key = new StringBuffer()
							...
							.toString();
					result.put(key, Bytes.toString(kv.getValue()));
				}
				results.add(result);
				printList(results);
			}
			...
			for (Result r : rs) {
				...
				for (KeyValue kv : r.raw()) {
					key = new StringBuffer()
							...
							.append(Bytes.toString(kv.getQualifier()))
							.toString();
					m.put(key, Bytes.toString(kv.getValue()));
				}
				results.add(m);
				printList(results);
			}
